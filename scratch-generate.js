...
const TARGET_SY_LABEL = "${label}";
const BASE_YEAR = ${year};
...
for (const y of years) {
  const nextYear = y + 1;
  const seedFile = path.join(__dirname, `server/prisma/seeds/seed-g7-${y}-${nextYear}.ts`);
  const wipeFile = path.join(__dirname, `server/prisma/seeds/wipe-g7-${y}-${nextYear}.ts`);
  fs.writeFileSync(seedFile, generateSeed(y));
  fs.writeFileSync(wipeFile, generateWipe(y));
  console.log(`Created ${seedFile} and ${wipeFile}`);
}
