...
The current implementation does not enforce every item in this contract. In
particular, SF6 enforcement is disabled, SF5 may be bypassed for some finalized
or empty sections, and a missing target year may be generated by shifting source
dates. These are release-safety gaps, not approved rollover behavior. See the
[code-verified safety section](../integration/ENROLLPRO-SCHOOL-YEAR-LIFECYCLE.md#code-verified-release-safety-gaps).
...
10. Activates the target year and applies its calendar under the coordinated contract.
