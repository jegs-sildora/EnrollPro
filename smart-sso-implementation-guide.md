# SMART SSO Implementation Guide (Node.js / Express)

Based on the contract in `SMART-ENROLLPRO-SSO.md`, here is a complete, production-ready Node.js/Express implementation template for the SMART backend. 
...
> [!IMPORTANT]
> **Server-Side Only**: This code must run on your backend server. Never expose `SMART_SSO_CLIENT_SECRET` to the frontend browser or attempt this exchange from client-side JavaScript.
...
// Environment variables required by SMART
const ENROLLPRO_BASE_URL = process.env.ENROLLPRO_BASE_URL || 'https://enrollpro.example.com';
const SMART_SSO_CLIENT_SECRET = process.env.SMART_SSO_CLIENT_SECRET;
...
router.get('/auth/enrollpro/callback', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const exchangeResponse = await axios.post(
      `${ENROLLPRO_BASE_URL}/api/auth/companion-sso/smart/exchange`,
      { code },
      {
        headers: {
          'Authorization': `Bearer ${SMART_SSO_CLIENT_SECRET}`,
    ...
    if (!hasValidRole) {
      ...
      return res.redirect(`${ENROLLPRO_BASE_URL}/login?error=Unauthorized+role+for+SMART`);
    }

    // 3. Map Identity & Create SMART Session
    // In your actual SMART database, you would find or update the user where externalId = subject
    const smartUser = await reconcileSmartUser({
      subjectId: subject, // Stable external identifier
      employeeId, 
      name
    });

    // Create a SMART-owned HTTP-only, Secure, SameSite session
    // (Example assumes express-session or similar is configured on `req.session`)
    req.session.userId = smartUser.id; 
    
    // Log success (NO secrets, NO tokens logged)
    console.info(`[SSO Success] EnrollPro Subject: ${subject} mapped to SMART Account: ${smartUser.id}`);

    // 4. Role-based Routing (clears code from browser history)
    if (roles.includes('SYSTEM_ADMIN')) {
      return res.redirect('https://laptop-pfvh73qk.buru-degree.ts.net/admin');
    } else {
      // HEAD_REGISTRAR, TEACHER, and CLASS_ADVISER
      return res.redirect('/workspace'); // Or your dynamic SMART workspace URL
    }
...
/**
 * Mock function to represent how SMART should map the identity
 */
async function reconcileSmartUser(data: { subjectId: string, employeeId: string, name: string }) {
...
> [!CAUTION]
> **Logging Restrictions**: As mandated by the contract, you must ensure that your global request loggers (like Morgan or Winston) do NOT log the `req.query.code`, the `SMART_SSO_CLIENT_SECRET` in headers, or the full `exchangeResponse.data` identity payload.
