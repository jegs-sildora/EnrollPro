...
async function main() {
  ...
  for (let i = 1; i <= totalLearners; i++) {
    ...
    const midIdx =
      Math.floor(i / (firstPool.length * PH_LAST_NAMES.length)) %
    ...
    const learnerData = {
      ...
      sex,
      birthdate: toUtcNoon(
        2013 - (parseInt(gradeLevel.name.split(" ")[1]) - 7),
        4,
        15 + (i % 15),
      ),
          ...
          create: [
            {
              ...
              firstName:
                PH_FIRST_NAMES_FEMALE[
                  (i + 1000) % PH_FIRST_NAMES_FEMALE.length
                ],
              lastName:
                PH_LAST_NAMES[
                  Math.floor((i + 1000) / PH_FIRST_NAMES_FEMALE.length) %
    ...
    if (sections.length > 0) {
      ...
      await prisma.enrollmentRecord.upsert({
        ...
        create: {
          enrollmentApplication: { connect: { id: application!.id } },
          schoolYear: { connect: { id: targetYear.id } },
          section: { connect: { id: section.id } },
          enrolledBy: { connect: { id: admin.id } },
