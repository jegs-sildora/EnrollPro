...
async function main() {
  console.log("🌱 Seeding EOSY mock grades for 2027-2028...");

  const activeSchoolYear = await prisma.schoolYear.findFirst({
    where: { yearLabel: "2027-2028" },
  });

  if (!activeSchoolYear) {
    console.error("❌ No school year found for 2027-2028.");
    return;
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    console.log(`✅ Successfully seeded mock EOSY data for ${totalSeeded} learners in 2027-2028.`);
  });
}

main()
  .catch((e) => {
    console.error("❌ Failed to seed EOSY mock data for 2027-2028. Transaction rolled back.", e);
    process.exit(1);
  })
  .finally(async () => {
