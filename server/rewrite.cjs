...
export async function submitAdmission(req: Request, res: Response) {
  try {
    ...
    res.status(201).json({
      ...
      id: admission.id,
      ...buildTrackingState("PENDING_VERIFICATION", data.scpType as any),
    });
  } catch (error) {
  ...
  try {
    ...
    const application = await prisma.enrollmentApplication.create({
      data: {
        ...
        applicantType: assignedProgram as any,
...
const startIdx = code.indexOf('export async function submitApplication');
const endIdx = code.indexOf('export async function updateExistingApplication');
