 ...
 * Tests all aspects of the Smart Academic Year Configuration Module
 */
...
const schedule2026 = deriveAcademicYearScheduleFromOpeningDate(new Date('2026-06-01'));
...
console.log(`  Early Reg Close: ${schedule2026.earlyRegCloseDate.toISOString()}`);
...
console.assert(schedule2026.yearLabel === '2026-2027', '❌ Year label should be 2026-2027');
console.assert(schedule2026.classEndDate.getUTCFullYear() === 2027, '❌ Class end should be in 2027');
console.assert(schedule2026.classEndDate.getUTCMonth() === 2, '❌ Class end should be March');
console.assert(schedule2026.classEndDate.getUTCDate() === 31, '❌ Class end should be March 31');
console.log('  ✅ PASSED\n');

// Test 2.2: Derive next academic year from current date
console.log('Test 2.2: Derive next academic year from today');
const nextYear = deriveNextAcademicYear(new Date());
console.log(`  Next Year Label: ${nextYear.yearLabel}`);
console.log(`  Class Opening: ${nextYear.classOpeningDate.toISOString()}`);
console.assert(nextYear.yearLabel.includes('-'), '❌ Should have year label format');
console.assert(nextYear.classOpeningDate.getUTCDay() === 1, '❌ Opening should be Monday');
console.log('  ✅ PASSED\n');

// Test 2.3: Custom class end date
console.log('Test 2.3: Custom class end date (April 15 instead of March 31)');
const customSchedule = deriveAcademicYearScheduleFromOpeningDate(
  new Date('2026-06-01'),
  new Date('2027-04-15')
);
console.log(`  Class End: ${customSchedule.classEndDate.toISOString()}`);
console.assert(customSchedule.classEndDate.getUTCMonth() === 3, '❌ Should be April');
console.assert(customSchedule.classEndDate.getUTCDate() === 15, '❌ Should be 15th');
console.log('  ✅ PASSED\n');
...
const overrideYear = createMockAcademicYear({ manualOverrideOpen: true });
console.assert(isEnrollmentOpen(overrideYear) === true, '❌ Should be open with override');
console.assert(getEnrollmentPhase(overrideYear) === 'OVERRIDE', '❌ Phase should be OVERRIDE');
console.log('  ✅ PASSED\n');

// Test 3.2: Early Registration Phase (Active)
console.log('Test 3.2: Early Registration Phase - Currently active');
const earlyRegYear = createMockAcademicYear({
...
console.assert(isEnrollmentOpen(earlyRegYear) === true, '❌ Should be open during early reg');
console.assert(getEnrollmentPhase(earlyRegYear) === 'EARLY_REGISTRATION', '❌ Phase should be EARLY_REGISTRATION');
console.log('  ✅ PASSED\n');

// Test 3.3: Regular Enrollment Phase (Active)
console.log('Test 3.3: Regular Enrollment Phase - Currently active');
const regularEnrollYear = createMockAcademicYear({
...
console.assert(isEnrollmentOpen(regularEnrollYear) === true, '❌ Should be open during regular enrollment');
console.assert(getEnrollmentPhase(regularEnrollYear) === 'REGULAR_ENROLLMENT', '❌ Phase should be REGULAR_ENROLLMENT');
console.log('  ✅ PASSED\n');

// Test 3.4: Closed (Between phases)
console.log('Test 3.4: Closed - Between phases');
const closedYear = createMockAcademicYear({
  earlyRegOpenDate: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000), // 60 days ago
  earlyRegCloseDate: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), // 30 days ago
  enrollOpenDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days from now
  enrollCloseDate: new Date(Date.now() + 37 * 24 * 60 * 60 * 1000), // 37 days from now
});
console.assert(isEnrollmentOpen(closedYear) === false, '❌ Should be closed between phases');
console.assert(getEnrollmentPhase(closedYear) === 'CLOSED', '❌ Phase should be CLOSED');
console.log('  ✅ PASSED\n');

// Test 3.5: Closed (After all phases)
console.log('Test 3.5: Closed - After all phases');
const pastYear = createMockAcademicYear({
...
console.assert(isEnrollmentOpen(pastYear) === false, '❌ Should be closed after all phases');
console.assert(getEnrollmentPhase(pastYear) === 'CLOSED', '❌ Phase should be CLOSED');
console.log('  ✅ PASSED\n');

// Test 3.6: Null dates handling
console.log('Test 3.6: Null dates - Should be closed');
const nullDatesYear = createMockAcademicYear({
  earlyRegOpenDate: null,
  earlyRegCloseDate: null,
  enrollOpenDate: null,
  enrollCloseDate: null,
});
console.assert(isEnrollmentOpen(nullDatesYear) === false, '❌ Should be closed with null dates');
console.assert(getEnrollmentPhase(nullDatesYear) === 'CLOSED', '❌ Phase should be CLOSED');
console.log('  ✅ PASSED\n');
...
console.assert(normalized.getUTCMilliseconds() === 0, '❌ Milliseconds should be 0');
console.log('  ✅ PASSED\n');
...
const decSchedule = deriveAcademicYearScheduleFromOpeningDate(new Date('2026-12-01'));
console.log(`  Year Label: ${decSchedule.yearLabel}`);
console.assert(decSchedule.yearLabel === '2026-2027', '❌ Should handle December correctly');
console.log('  ✅ PASSED\n');

// Test 5.2: Enrollment phases overlap prevention
console.log('Test 5.2: Phases should not overlap');
const testSchedule = deriveAcademicYearScheduleFromOpeningDate(new Date('2026-06-01'));
const earlyRegEnd = testSchedule.earlyRegCloseDate.getTime();
const regularEnrollStart = testSchedule.enrollOpenDate.getTime();
console.assert(earlyRegEnd < regularEnrollStart, '❌ Early reg should end before regular enrollment');
console.log('  ✅ PASSED\n');
...
console.assert(enrollClose < classOpen, '❌ Enrollment should close before classes open');
console.log('  ✅ PASSED\n');
...
console.log('\n' + '='.repeat(70));
console.log('✅ ALL TESTS PASSED - School Year Setup Architecture is working correctly!');
console.log('='.repeat(70));
console.log('\nKey Features Verified:');
console.log('  ✓ DepEd calendar date computation (First Monday of June, etc.)');
console.log('  ✓ Academic year schedule derivation');
