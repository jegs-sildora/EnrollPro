...
export async function getLearnerDashboardUnified(req: Request, res: Response): Promise<void> {
  ...
  const activeGradeLevel = activeApp?.gradeLevel?.name;

  const filteredLocalHistory = localHistory.filter((h) => {
    if (smartYears.has(h.school_year)) return false;
    if (smartGrades.has(h.grade_level) && h.grade_level !== activeGradeLevel) return false;
    return true;
  });
