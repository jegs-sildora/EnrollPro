...
export function getRequiredDocuments(params: {
  ...
  const { learnerType, gradeLevel, applicantType, isLwd, isPeptAePasser } = params;
  ...
  if (learnerType !== "CONTINUING") {
    requirements.push({
      ...
      isRequired: true,
      isOnceOnly: true,
      description: "Required once per school stay. Secondary proof accepted if unavailable.",
      phase: 1,
    });
  }
  ...
  if (learnerType === "NEW_ENROLLEE") {
    ...
    requirements.push({
      ...
      isRequired: true,
      isOnceOnly: false,
      description: "SF9, certification, or any prior record (flexible for Balik-Aral)",
      phase: 1,
    });
  }
  ...
  if (learnerType === "TRANSFEREE") {
    requirements.push({
      ...
      isOnceOnly: false,
      description: "Required only if there are unpaid fees at the previous private school",
      phase: 2,
    });
  }
