...
- `server/`: Express 5 + Prisma backend. Entry points are `server/src/app.ts` and `server/src/server.ts`; domain code is under `features/`, utilities under `lib/` and `middleware/`, Prisma files under `prisma/`, and tests under `server/src/tests`.
...
Backend tests are TypeScript files named `*.test.ts` under `server/src/tests`; root-level `server/test_*.ts` scripts are ad hoc checks. Add focused coverage near changed domain behavior, especially enrollment lifecycle, rollover, integration, sectioning, audit, and Prisma workflows.

Run targeted backend tests from `server/` with `tsx src/tests/<name>.test.ts`, or use `pnpm --filter server test:integration`. For frontend changes, run `pnpm --filter client lint` and `pnpm --filter client build` when practical.
