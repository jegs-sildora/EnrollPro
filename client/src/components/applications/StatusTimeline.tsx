...
export function StatusTimeline({ applicant }: Props) {
  ...
  return (
    <div className="mt-4 p-4 border-t">
      <h3 className="text-xs font-bold text-muted-foreground tracking-wider mb-3 uppercase">Timeline</h3>
      <div className="space-y-3">
        {timeline.map((event, idx) => (
          <div key={idx} className="flex gap-3 items-start">
            <div className="flex flex-col items-center pt-1">
              <div className="w-2 h-2 rounded-full bg-muted-foreground" />
              {idx < timeline.length - 1 && <div className="w-px h-full bg-border mt-1" />}
            </div>
            <div className="text-sm">
              <p className="font-medium text-foreground">{event.label}</p>
              <p className="text-xs text-muted-foreground">{format(event.date, 'MMM d, yyyy h:mm a')}</p>
            </div>
          </div>
        ))}
      </div>
