...
import { admissionSchema, type AdmissionFormData } from '../apply/types';
...
import Step5Enrollment from '../apply/components/Step5Enrollment';
import Step6Review from '../apply/components/Step6Review';
import F2FAdmissionSuccess from './F2FAdmissionSuccess';
import F2FStepProgressBar from './F2FStepProgressBar';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { motion, AnimatePresence } from 'motion/react';
import { ArrowLeft, ArrowRight, AlertCircle, UserPlus, RotateCcw } from 'lucide-react';
...
export default function F2FAdmission() {
  ...
  useEffect(() => {
    ...
    if (draft) {
      try {
        ...
        console.error('Failed to parse F2F admission draft:', e);
      }
    }
  ...
  const onSubmit = async (data: AdmissionFormData) => {
    ...
    try {
      ...
      const message = (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Failed to submit F2F application. Please try again.';
  ...
  if (isSubmitted) {
    return <F2FAdmissionSuccess trackingNumber={trackingNumber} encodedBy={user?.name || 'Staff'} onNewApplication={handleFullReset} />;
  }
        ...
        <CardHeader className="px-0 pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-xl font-bold">Walk-in Admission</CardTitle>
                <CardDescription className="text-sm">
                  Encode face-to-face admission application
                </CardDescription>
              </div>
            </div>
            <Button
