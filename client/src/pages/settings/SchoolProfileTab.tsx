...
export default function SchoolProfileTab() {
  const { schoolName, logoUrl, colorScheme, selectedAccentHsl, setSettings } = useSettingsStore();
  ...
  const palette: PaletteColor[] = (colorScheme as { palette?: PaletteColor[] } | null)?.palette ?? [];
  const currentAccent = selectedAccentHsl ?? (colorScheme as { accent_hsl?: string } | null)?.accent_hsl ?? '221 83% 53%';
  ...
  const handleSaveName = async () => {
    ...
    try {
      ...
      sileo.success({ title: 'Settings Saved', description: 'School name updated.' });
    } catch (err) {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      sileo.error({ title: 'File too large', description: 'Maximum file size is 2MB.' });
      return;
    }
    ...
    try {
      ...
      setTimeout(() => {
        sileo.success({ title: 'Logo Uploaded', description: 'Palette extracted from your logo.' });
      }, 50);
    } catch (err) {
      setLogoPreview(null);
      toastApiError(err as never);
  ...
  const handleRemoveLogo = async () => {
    ...
    try {
      ...
      setSettings({ logoUrl: null, colorScheme: null, selectedAccentHsl: null });
      setLogoPreview(null);
      setTimeout(() => {
        sileo.success({ title: 'Logo Removed', description: 'Default blue accent restored.' });
      }, 50);
    } catch (err) {
      toastApiError(err as never);
    } finally {
  ...
  const handleSelectAccent = async (color: PaletteColor) => {
    ...
    try {
      ...
      setTimeout(() => {
        sileo.success({ title: 'Accent Updated', description: 'Your accent color has been changed.' });
      }, 50);
    } catch (err) {
      toastApiError(err as never);
    } finally {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>The name displayed across the application</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <Button onClick={handleSaveName} disabled={savingName || nameValue === schoolName}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload a logo to extract a color palette. Choose your accent color from the extracted palette.
          </CardDescription>
        </CardHeader>
