...
export default function Students() {
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      ...
      try {
        ...
        const allSections = (secRes.data.gradeLevels || []).flatMap((gl: ApiGradeLevelGroup) => 
  ...
  useEffect(() => {
    if (gradeLevelFilter === 'all') {
      ...
      setFilteredSections(sections.filter(s => s.gradeLevelId === parseInt(gradeLevelFilter, 10)));
    }
                ...
                {loading ? (
                  ...
                  students.map((student) => (
                    <TableRow key={student.id}>
                      ...
                      <TableCell className="hidden lg:table-cell">{student.strand || '—'}</TableCell>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-xs text-[hsl(var(--muted-foreground))]">Full Name</Label>
                    <p className="font-medium">{selectedStudent.fullName}</p>
                  </div>
                  <div>
                    <Label className="text-xs text-[hsl(var(--muted-foreground))]">LRN</Label>
                    <p className="font-mono text-sm">{selectedStudent.lrn}</p>
                  </div>
                  <div>
                    <Label className="text-xs text-[hsl(var(--muted-foreground))]">Date of Birth</Label>
                    <p className="text-sm">{formatDate(selectedStudent.birthDate)} ({calculateAge(selectedStudent.birthDate)} yrs)</p>
                  </div>
                  <div>
                    <Label className="text-xs text-[hsl(var(--muted-foreground))]">Sex</Label>
                    <p className="text-sm">{selectedStudent.sex}</p>
                  </div>
                  <div className="col-span-2">
                    <Label className="text-xs text-[hsl(var(--muted-foreground))]">Home Address</Label>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-xs text-[hsl(var(--muted-foreground))]">Parent/Guardian</Label>
                    <p className="text-sm">{selectedStudent.parentGuardianName}</p>
                  </div>
                  <div>
                    <Label className="text-xs text-[hsl(var(--muted-foreground))]">Contact Number</Label>
                    <p className="text-sm font-mono">{selectedStudent.parentGuardianContact}</p>
                  </div>
                  <div className="col-span-2">
                    <Label className="text-xs text-[hsl(var(--muted-foreground))]">Email Address</Label>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-xs text-[hsl(var(--muted-foreground))]">Academic Year</Label>
                    <p className="text-sm">{selectedStudent.academicYear}</p>
                  </div>
                  <div>
                    <Label className="text-xs text-[hsl(var(--muted-foreground))]">Grade Level</Label>
                    <p className="text-sm">{selectedStudent.gradeLevel}</p>
                  </div>
                  {selectedStudent.strand && (
                    <div>
                      <Label className="text-xs text-[hsl(var(--muted-foreground))]">Strand</Label>
                  ...
                  {selectedStudent.enrollment && (
                    <>
                      <div>
                        <Label className="text-xs text-[hsl(var(--muted-foreground))]">Section</Label>
                        <p className="text-sm">{selectedStudent.enrollment.section}</p>
                      </div>
                      {selectedStudent.enrollment.advisingTeacher && (
                        <div>
                          <Label className="text-xs text-[hsl(var(--muted-foreground))]">Advising Teacher</Label>
                          <p className="text-sm">{selectedStudent.enrollment.advisingTeacher}</p>
                        </div>
                      )}
                      <div>
                        <Label className="text-xs text-[hsl(var(--muted-foreground))]">Enrolled On</Label>
                        <p className="text-sm">{formatDate(selectedStudent.enrollment.enrolledAt)}</p>
                      </div>
                      <div>
                        <Label className="text-xs text-[hsl(var(--muted-foreground))]">Enrolled By</Label>
                        <p className="text-sm">{selectedStudent.enrollment.enrolledBy}</p>
                      </div>
                    </>
                  )}
                </div>
              ...
              {selectedStudent.status === 'REJECTED' && selectedStudent.rejectionReason && (
                <div className="space-y-2 p-4 bg-red-50 rounded-lg border border-red-200">
                  ...
                  <p className="text-sm text-red-900">{selectedStudent.rejectionReason}</p>
                </div>
              )}
