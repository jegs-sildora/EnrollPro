    ...
    <Card className="lg:hidden border-0 shadow-lg shadow-slate-200/40 rounded-[2rem] overflow-hidden bg-muted">
      <CardHeader className="p-4 border-b border-slate-100 flex flex-row items-center justify-between">
        ...
        <Select value={selectedTerm} onValueChange={(val) => val && onTermChange(val)}>
          <SelectTrigger className="h-10 w-24 bg-muted border-slate-200 text-xs font-bold uppercase rounded-xl shadow-sm px-3">
            <SelectValue />
          </SelectTrigger>
          <SelectContent className="rounded-xl border-slate-200 shadow-2xl p-2">
            {terms.map((q) => (
              <SelectItem
                key={q}
                value={q}
                className="text-xs font-bold uppercase rounded-lg py-2 px-4 focus:bg-indigo-50 focus:text-indigo-600 transition-colors cursor-pointer"
              >
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-500  mt-0.5 font-mono">{record.student.lrn}</p>

                  {!isHGClass && hasScores && (
                    <div className="flex items-center gap-3 mt-2">
                      <div className="flex items-center gap-1">
                        <span className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest">WW</span>
                      ...
                      <div className="flex items-center gap-1">
                        <span className="text-[9px] font-bold text-purple-400 uppercase tracking-widest">PT</span>
                      ...
                      <div className="w-px h-3 bg-slate-200" />
                      <div className="flex items-center gap-1">
                        <span className="text-[9px] font-bold text-amber-400 uppercase tracking-widest">TA</span>
                ...
                <div className="text-right flex-shrink-0">
                  {isHGClass ? (
                    <>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Descriptor</p>
