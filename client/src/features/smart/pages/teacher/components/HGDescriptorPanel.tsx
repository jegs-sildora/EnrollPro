  ...
  return (
    <Card className="hidden lg:block border-0 shadow-2xl shadow-slate-200/40 rounded-[2.5rem] overflow-hidden bg-muted">
      <CardHeader className="p-8 border-0 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight uppercase">Homeroom Guidance Descriptors</h2>
        ...
        <div className="flex items-center gap-4">
          ...
          <Select value={selectedTerm} onValueChange={(val) => val && onTermChange(val)}>
            <SelectTrigger className="h-11 w-40 bg-muted border-slate-200 text-sm font-bold uppercase rounded-xl shadow-sm px-6">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="rounded-xl border-slate-200 shadow-2xl p-2">
              {terms.map((q) => (
                <SelectItem key={q} value={q} className="text-xs font-bold uppercase rounded-lg py-2.5 px-4 focus:bg-indigo-50 focus:text-indigo-600 transition-colors cursor-pointer">
                  {q}
                </SelectItem>
              ))}
            </SelectContent>
