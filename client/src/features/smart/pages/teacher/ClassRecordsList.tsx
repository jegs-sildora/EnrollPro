        ...
        <Card className={`h-full ${containerClass} transition-all duration-500 ${archived ? 'rounded-[2rem]' : 'rounded-[2.5rem]'} ${cardBg} overflow-hidden flex flex-col relative group-hover:-translate-y-2`}>
          ...
          <CardHeader className={`${archived ? 'p-6 pb-3' : 'p-8 pb-4'} relative z-10`}>
            ...
            <div className="space-y-1">
              ...
              <h3 className={`${archived ? 'text-lg' : 'text-2xl'} font-bold text-slate-900 ${titleHover} transition-colors leading-tight`}>
          ...
          <CardContent className={`${archived ? 'p-6 pt-4' : 'p-8 pt-6'} mt-auto relative z-10`}>
            <div className={`flex items-center justify-between ${archived ? 'pt-4' : 'pt-6'} border-t ${archived ? "border-rose-100" : "border-slate-50"}`}>
              <div className={`flex items-center ${archived ? 'gap-2' : 'gap-3'}`}>
                ...
                <div>
                  ...
                  <p className={`${archived ? 'text-xs' : 'text-sm'} font-bold text-slate-900`}>{assignment.section.enrollments?.length || 0} Learners</p>
                </div>
              </div>

              <div className="text-right">
                <p className={`text-[10px] font-bold uppercase tracking-widest ${mutedText}`}>Weights</p>
                <p className={`${archived ? 'text-[10px]' : 'text-xs'} font-bold text-slate-900 font-mono tracking-tighter`}>
            ...
            <div className={`${archived ? 'mt-3' : 'mt-4'} flex items-center justify-between`}>
              <Badge className={`${archived ? "bg-rose-100 text-rose-700 border-rose-200" : "bg-slate-100 text-slate-500 border-0"} text-[10px] font-bold uppercase tracking-widest ${archived ? 'px-2' : 'px-3'}`}>
    ...
    <div className="space-y-10 animate-fade-in max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div className="space-y-2">
          ...
          <h1 className="text-4xl font-bold text-slate-900 tracking-tight">Class Records</h1>
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-center">
            <div className="relative flex-1 w-full group">
              ...
              <Input
                ...
                className="pl-16 h-14 bg-slate-50/50 border-0 hover:bg-slate-50 focus:bg-muted focus:ring-4 focus:ring-indigo-50 rounded-2xl text-base font-bold transition-all placeholder:text-slate-400"
              />
            </div>
            ...
            <div className="space-y-6">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900">Archived Class Records</h2>
                    ...
                    <div className="p-8 hover:bg-slate-50/50 transition-all duration-300 flex flex-col sm:flex-row sm:items-center gap-8 group">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-3 mb-2">
                          <h3 className="text-xl font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{assignment.subject.name}</h3>
                      ...
                      <div className="flex items-center gap-12">
                        <div className="text-center">
                          <p className="text-2xl font-bold text-slate-900 leading-none">{assignment.section.enrollments?.length || 0}</p>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-2">Learners</p>
                        </div>

                        <div className="hidden lg:block">
                          <div className="px-5 py-3 rounded-2xl bg-slate-50 border border-slate-100 group-hover:bg-muted transition-colors">
                            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-1 text-center">WW / PT / TA</p>
                            <p className="text-sm text-slate-900 font-bold font-mono tracking-tighter text-center">
                    ...
                    {archivedClasses.map((assignment) => (
                      <Link key={assignment.id} to={`/teacher/records/${assignment.id}`} className="block group">
                        <div className="p-8 hover:bg-rose-50 transition-all duration-300 flex flex-col sm:flex-row sm:items-center gap-8 group">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-3 mb-2">
                              <h3 className="text-xl font-bold text-slate-900 group-hover:text-rose-600 transition-colors">{assignment.subject.name}</h3>
                            ...
                            {assignment.archivedReason && (
                              <p className="mt-3 text-sm  text-rose-700 bg-rose-50 border border-rose-100 rounded-2xl px-4 py-3">
                                {assignment.archivedReason}
                              </p>
                            )}
                          </div>
      ...
      {activeClasses.length === 0 && archivedClasses.length === 0 && (
        <Card className="border-0 shadow-2xl shadow-slate-200/40 rounded-[2.5rem] bg-muted overflow-hidden">
          <CardContent className="py-32 text-center">
            ...
            <h3 className="font-bold text-slate-900 text-2xl mb-3">No Classes Found</h3>
