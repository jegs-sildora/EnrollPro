        ...
        <Card className={`h-full ${containerClass} transition-all duration-500 ${archived ? 'rounded-[2rem]' : 'rounded-[2.5rem]'} ${cardBg} overflow-hidden flex flex-col relative group-hover:-translate-y-2`}>
          ...
          <CardHeader className={`${archived ? 'p-6 pb-3' : 'p-8 pb-4'} relative z-10`}>
            <div className={`flex items-start justify-between ${archived ? 'mb-4' : 'mb-8'}`}>
              <Badge className={`${badgeClass} text-[10px] font-extrabold uppercase tracking-[0.1em] ${archived ? 'px-3 py-1' : 'px-4 py-1.5'} rounded-full`}>
            ...
            <div className="space-y-1">
              <p className={`text-[10px] font-extrabold uppercase tracking-[0.2em] ${mutedText}`}>Subject Title</p>
              <h3 className={`${archived ? 'text-lg' : 'text-2xl'} font-extrabold text-slate-900 ${titleHover} transition-colors leading-tight`}>
            ...
            <div className={`${archived ? 'pt-3' : 'pt-4'} flex items-center gap-2`}>
              ...
              <p className={`${archived ? 'text-xs' : 'text-sm'} font-extrabold ${sectionText}`}>
                Section {assignment.section.name} &bull; {assignment.schoolYear}
              </p>
            </div>

            {archived && assignment.archivedReason && (
              <p className="mt-3 text-[10px] font-extrabold text-rose-600 bg-rose-50 rounded-xl px-3 py-2 border border-rose-100 leading-relaxed line-clamp-2">
          ...
          <CardContent className={`${archived ? 'p-6 pt-4' : 'p-8 pt-6'} mt-auto relative z-10`}>
            <div className={`flex items-center justify-between ${archived ? 'pt-4' : 'pt-6'} border-t ${archived ? "border-rose-100" : "border-slate-50"}`}>
              <div className={`flex items-center ${archived ? 'gap-2' : 'gap-3'}`}>
                ...
                <div>
                  <p className={`text-[10px] font-extrabold uppercase tracking-widest ${mutedText}`}>Enrolled</p>
                  <p className={`${archived ? 'text-xs' : 'text-sm'} font-extrabold text-slate-900`}>{assignment.section.enrollments?.length || 0} Learners</p>
                </div>
              </div>

              <div className="text-right">
                <p className={`text-[10px] font-extrabold uppercase tracking-widest ${mutedText}`}>Weights</p>
                <p className={`${archived ? 'text-[10px]' : 'text-xs'} font-extrabold text-slate-900 font-mono tracking-tighter`}>
            ...
            <div className={`${archived ? 'mt-3' : 'mt-4'} flex items-center justify-between`}>
              <Badge className={`${archived ? "bg-rose-100 text-rose-700 border-rose-200" : "bg-slate-100 text-slate-500 border-0"} text-[10px] font-extrabold uppercase tracking-widest ${archived ? 'px-2' : 'px-3'}`}>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div className="space-y-2">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Badge variant="secondary" className="bg-indigo-50 text-indigo-700 border-indigo-100 text-[10px] font-extrabold uppercase tracking-widest px-3">
              {activeClasses.length} ACTIVE CLASSES
            </Badge>
            {hasArchived && (
              <Badge className="bg-rose-600 text-white border-rose-200 text-[10px] font-extrabold uppercase tracking-widest px-3 shadow-lg shadow-rose-100">
                {archivedClasses.length} BACKUP ARCHIVED
              </Badge>
            )}
          </div>
          <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">Class Records</h1>
          <p className="text-slate-500  text-lg">Select a section to manage student performance and mastery</p>
        </div>
      </div>
        ...
        <CardContent className="p-8">
          <div className="flex flex-col lg:flex-row gap-6 items-center">
            ...
            <div className="flex items-center gap-4 w-full lg:w-auto">
              <Button variant="outline" className="h-14 px-8 rounded-2xl border-slate-200 text-slate-600 font-extrabold hover:bg-slate-50 transition-all flex-1 lg:flex-none">
            ...
            <div className="space-y-6">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-extrabold text-slate-900">Archived Class Records</h2>
                    <p className="text-slate-500 ">Backup data from previous syncs or manual archives</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  {isArchivedExpanded && (
                    <Button
                      variant="destructive"
                      size="sm"
                      className="rounded-xl font-extrabold bg-rose-600 hover:bg-rose-700 text-white h-10 px-4 shadow-lg shadow-rose-100 animate-in fade-in slide-in-from-right-4 text-[10px] tracking-widest uppercase"
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <Badge className="bg-rose-50 text-rose-600 border-rose-100 font-extrabold px-4 py-2 rounded-xl">
                    ...
                    <div className="p-8 hover:bg-slate-50/50 transition-all duration-300 flex flex-col sm:flex-row sm:items-center gap-8 group">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-3 mb-2">
                          <h3 className="text-xl font-extrabold text-slate-900 group-hover:text-indigo-600 transition-colors">{assignment.subject.name}</h3>
                          <Badge className={`${getGradeColors(assignment.section.gradeLevel).badge} border-0 text-[10px] font-extrabold uppercase tracking-widest px-3`}>
                      ...
                      <div className="flex items-center gap-12">
                        <div className="text-center">
                          <p className="text-2xl font-extrabold text-slate-900 leading-none">{assignment.section.enrollments?.length || 0}</p>
                          <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-[0.2em] mt-2">Learners</p>
                        </div>
                    ...
                    {archivedClasses.map((assignment) => (
                      <Link key={assignment.id} to={`/teacher/records/${assignment.id}`} className="block group">
                        <div className="p-8 hover:bg-rose-50 transition-all duration-300 flex flex-col sm:flex-row sm:items-center gap-8 group">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-3 mb-2">
                              <h3 className="text-xl font-extrabold text-slate-900 group-hover:text-rose-600 transition-colors">{assignment.subject.name}</h3>
                              <Badge className="bg-rose-100 text-rose-700 border-rose-200 text-[10px] font-extrabold uppercase tracking-widest px-3">
                                ARCHIVED
                              </Badge>
                            </div>
                            <p className="text-rose-500 font-extrabold text-sm uppercase tracking-widest">
                              Section {assignment.section.name} &bull; {assignment.schoolYear}
                            </p>
                            {assignment.archivedReason && (
                          ...
                          <div className="flex items-center gap-12">
                            <div className="text-center">
                              <p className="text-2xl font-extrabold text-slate-900 leading-none">{assignment.section.enrollments?.length || 0}</p>
                              <p className="text-[10px] font-extrabold text-rose-400 uppercase tracking-[0.2em] mt-2">Learners</p>
                            </div>
      ...
      {activeClasses.length === 0 && archivedClasses.length === 0 && (
        <Card className="border-0 shadow-2xl shadow-slate-200/40 rounded-[2.5rem] bg-muted overflow-hidden">
          <CardContent className="py-32 text-center">
            ...
            <h3 className="font-extrabold text-slate-900 text-2xl mb-3">No Classes Found</h3>
        ...
        <DialogContent className="w-full max-w-3xl rounded-[2rem] border-0 shadow-2xl p-0 overflow-hidden">
          <div className="bg-rose-600 p-8 text-white">
            ...
            <DialogHeader className="p-0 text-left">
              <DialogTitle className="text-2xl font-extrabold text-white leading-tight">Are You sure to delete this Subject?</DialogTitle>
              <DialogDescription className="text-rose-100  text-base mt-2">
                Permanently removing <span className="font-extrabold text-white underline decoration-2 underline-offset-4">{assignmentToDelete?.name}</span> will erase all student grades and synchronization history for this record.
              </DialogDescription>
            </DialogHeader>
          </div>
          <DialogFooter className="p-8 bg-muted flex flex-col sm:flex-row gap-4">
            <Button
              ...
              className="h-14 rounded-2xl border-slate-200 font-extrabold hover:bg-slate-50 hover:text-slate-900 transition-all flex-1"
              disabled={isDeleting}
            >
              CANCEL
            </Button>
            <Button
              ...
              className="h-14 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white font-extrabold shadow-xl shadow-rose-100 transition-all flex-1"
              disabled={isDeleting}
            >
              {isDeleting ? (
                "DELETING..."
      ...
      <Dialog open={confirmDeleteAll} onOpenChange={setConfirmDeleteAll}>
        <DialogContent className="w-full max-w-3xl rounded-[2rem] border-0 shadow-2xl p-0 overflow-hidden">
          <div className="bg-rose-600 p-8 text-white">
            ...
            <DialogHeader className="p-0 text-left">
              <DialogTitle className="text-2xl font-extrabold text-white leading-tight">Delete All Archived Records?</DialogTitle>
              <DialogDescription className="text-rose-100  text-base mt-2">
                This will permanently remove all <span className="font-extrabold text-white underline decoration-2 underline-offset-4">{archivedClasses.length} archived subjects</span>. This action cannot be undone and all historical grades for these records will be lost.
              </DialogDescription>
            </DialogHeader>
          </div>
          <DialogFooter className="p-8 bg-muted flex flex-col sm:flex-row gap-4">
