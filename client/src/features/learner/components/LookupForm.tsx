...
export function LookupForm({ onLookup, loading, error }: LookupFormProps) {
  ...
  const years = Array.from({ length: 50 }, (_, i) => String(currentYear - 3 - i));
  ...
  const isFormValid = lrn.length === 12 && pin.length === 6 && month && day && year;

  return (
    <Card className="w-full max-w-md mx-auto shadow-lg border-primary/10">
      <CardHeader className="text-center pb-2">
        <div className="flex justify-center mb-4">
           <img src={depedLogo} alt="DepEd Logo" className="h-20 w-auto object-contain" />
        </div>
        <CardTitle className="text-2xl font-bold tracking-tight">View Your School Records</CardTitle>
        <CardDescription className="text-sm">
          Enter your details below to access your SIMS record and health monitoring (SF8).
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-2">
            <Label htmlFor="lrn" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
          ...
          <div className="space-y-2">
            <Label htmlFor="pin" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
              Portal PIN *
            </Label>
            <Input
              id="pin"
          ...
          <div className="text-center pt-2 border-t">
            <button 
              ...
              onClick={() => alert("Please visit the school registrar in person with a valid ID or birth certificate to reset your PIN.")}
            >
