  ...
  return (
    <div className="p-6 h-full flex flex-col bg-background overflow-y-auto">
      <div className="mb-6 w-full flex items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold uppercase mb-2">Subject Deficiencies</h2>
          <p className="text-foreground font-bold w-full">
            This learner holds a <span className="font-extrabold">Conditionally Promoted</span> status. Please select an active class section for each subject deficiency below to complete the enrollment record.
          </p>
        </div>
      </div>
          ...
          <div key={def.id} className="border rounded-xl p-5 bg-card text-card-foreground shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all hover:border-primary/30">
            ...
            <div className="flex items-center gap-4 flex-1">
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-3">
                  <span className="font-extrabold uppercase text-lg">{def.gradeLevel} {def.subject}</span>
                  <Badge
                    variant="secondary"
                    className={cn(
                      "uppercase font-extrabold w-fit px-2.5 py-1 text-xs border-0",
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 flex-1 w-full justify-end">
              <div className="w-full flex justify-end">
                ...
                >
                  <SelectTrigger className="w-full h-11 border-2 font-extrabold text-left uppercase">
                    <SelectValue placeholder="Select Class Section" />
                  </SelectTrigger>
                  <SelectContent>
                    {MOCK_SECTIONS.filter((s) => s.gradeLevel === def.gradeLevel).map((section) => (
                      <SelectItem key={section.id} value={section.id} className="font-extrabold uppercase">
                        {section.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
