...
export default function AcademicYearLifecycleTab() {
  ...
  const handleUnlockBosy = async () => {
    if (unlockJustification.length < 10) {
      sileo.error({
        ...
        description: "Please provide a detailed justification for this emergency action.",
      });
      return;
    }
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      <Card className={cn("bg-white shadow-sm", isLocked ? "border-emerald-200" : "border-amber-200")}>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <CardDescription>
                Active Academic Year: <span className="font-bold text-foreground">{activeSchoolYearLabel}</span>
              </CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          {isLocked ? (
            <Alert className="bg-emerald-100/50 border-emerald-200 text-emerald-900">
              ...
              <AlertTitle className="font-bold">Academic Phase Active</AlertTitle>
              <AlertDescription className="text-sm font-medium">
                Official enrollment has ended. SF1 rosters are finalized and synchronized with A.T.L.A.S. and S.M.A.R.T. 
                New registrations are now classified as "Late Enrollees".
                {bosyLockedAt && (
            ...
            <Alert className="bg-amber-100/50 border-amber-200 text-amber-900">
              ...
              <AlertDescription className="text-sm font-medium">
                The system is currently in the Enrollment Phase. Registrars can perform batch sectioning and mass verifications. 
                Locking BOSY will finalize the official learner rosters (SF1).
              </AlertDescription>
            </Alert>
          )}
            ...
            <div className="p-4 rounded-xl border bg-white space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between border-b border-slate-50 pb-1">
                  <span className="text-sm font-bold text-slate-600">Learners Sectioned:</span>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-black">{preLockStats?.sectionedCount?.toLocaleString() ?? "..."}</span>
                    <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" />
                  </div>
                </div>
                
                <div className="flex items-center justify-between border-b border-slate-50 pb-1">
                  <span className="text-sm font-bold text-slate-600">Unsectioned (Ready):</span>
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-black ${preLockStats && preLockStats.unsectionedCount > 0 ? "text-red-600" : "text-slate-900"}`}>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold text-slate-600">Pending Verification:</span>
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-black ${preLockStats && preLockStats.pendingCount > 0 ? "text-red-600" : "text-slate-900"}`}>
            ...
            <div className="p-4 rounded-xl border bg-white space-y-3">
              ...
              <ul className="space-y-2">
                <li className="flex items-center gap-2 text-xs font-bold text-muted-foreground">
                  <div className={`h-1.5 w-1.5 rounded-full ${isLocked ? "bg-emerald-500" : "bg-slate-300"}`} />
        ...
        <CardFooter className="bg-white border-t p-6 flex flex-col items-start gap-4">
          {!isLocked ? (
                ...
                <DialogContent className="sm:max-w-[500px]">
                  ...
                  <div className="space-y-6 py-4">
                    <div className="bg-red-50 border border-red-200 p-4 rounded-lg space-y-2">
                      <p className="text-sm font-bold text-red-700 underline uppercase tracking-tight">Critical Impacts:</p>
                      <ul className="text-xs font-medium space-y-1 list-disc pl-4 text-red-800/80">
                        ...
                        <li>SF1 learner rosters will be finalized and sent to A.T.L.A.S. and S.M.A.R.T.</li>
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        <Label htmlFor="confirm-sy" className="text-xs font-black uppercase">Type the School Year to confirm:</Label>
                        <div className="relative">
                          ...
                          />
                          {lockConfirmLabel === activeSchoolYearLabel && lockConfirmLabel !== "" && (
                        ...
                        <div className="flex justify-between gap-2">
                          {pinDigits.map((digit, idx) => (
                            <Input
                              ...
                              type={visibleIndices.includes(idx) ? "text" : "password"}
                              inputMode="numeric"
                              maxLength={1}
                              value={digit}
                  ...
                  <DialogFooter>
                    <Button variant="ghost" onClick={() => setIsLockModalOpen(false)} disabled={isLocking}>
                      Cancel
                    </Button>
                    <Button 
                      className={cn(
                        "font-black uppercase tracking-widest transition-colors px-6",
                        (isLocking || lockConfirmLabel !== activeSchoolYearLabel || lockPin.length !== 6)
                          ? "bg-slate-300 text-slate-500 hover:bg-slate-300 cursor-not-allowed"
                          : "bg-red-600 text-white hover:bg-red-700 shadow-md"
                      )}
                      onClick={handleLockBosy}
                      disabled={isLocking || lockConfirmLabel !== activeSchoolYearLabel || lockPin.length !== 6}
                    >
              ...
              {isLockBlocked && (
                <div className="flex items-center gap-2 text-red-600 bg-red-50 px-4 py-2 rounded-lg border border-red-100 animate-in fade-in slide-in-from-left-2 duration-300">
                  ...
                  <p className="text-xs font-black uppercase tracking-tight">
                    Action Disabled: Resolve all {totalIncomplete} pending and unsectioned learners before locking.
                  </p>
                </div>
              )}
            </>
            ...
            <Dialog open={isUnlockModalOpen} onOpenChange={setIsUnlockModalOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" className="font-black uppercase tracking-widest border-emerald-200 text-emerald-700 hover:bg-emerald-50">
              ...
              <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                  ...
                  <DialogDescription className="font-bold text-foreground pt-2">
                    Reopening BOSY is a restricted action. This will re-enable all batch operations.
                  </DialogDescription>
                </DialogHeader>

                <div className="space-y-4 py-4">
                  <div className="space-y-2">
                    <Label htmlFor="unlock-reason" className="text-xs font-black uppercase text-emerald-700">Reason for Unlock (Mandatory):</Label>
                    <Textarea 
                      id="unlock-reason" 
                ...
                <DialogFooter>
                  <Button variant="ghost" onClick={() => setIsUnlockModalOpen(false)} disabled={isUnlocking}>
                    Cancel
                  </Button>
                  <Button 
      ...
      <div className="p-4 rounded-lg border border-primary/20 bg-primary/5 flex items-start gap-3">
        ...
        <div className="space-y-1">
          <p className="text-sm font-bold text-primary">Microservice Integration Status</p>
          <p className="text-xs text-slate-500 font-medium leading-relaxed">
            When BOSY is locked, EnrollPro automatically signals A.T.L.A.S. (Scheduling) and S.M.A.R.T. (Grading) 
            to ingest the finalized learner rosters. Late enrollees will be synced incrementally.
          </p>
        </div>
      </div>
    </div>
