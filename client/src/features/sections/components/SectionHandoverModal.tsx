...
export function SectionHandoverModal({
  ...
  const handleHandover = async () => {
    if (!substituteTeacherId || !handoverReason) {
      sileo.warning({
        ...
        description:
          "Please select a substitute teacher and a reason for handover.",
      });
      return;
    }
  ...
  const selectedSubstitute = teachers.find(
    (t) => t.id.toString() === substituteTeacherId,
  );
      ...
      <DialogContent className="w-full max-w-3xl border-2 p-0 overflow-hidden rounded-2xl bg-background">
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div className="rounded-xl border bg-muted/30 p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant="outline"
                className="text-sm font-bold uppercase border-amber-200 bg-amber-50 text-amber-700">
                ACTIVE
              </Badge>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-base uppercase leading-none">
                  ...
                  {isSearching ? (
                    ...
                    filteredTeachers.map((teacher) => (
                      <button
                        key={teacher.id}
                        onClick={() =>
                          setSubstituteTeacherId(teacher.id.toString())
                        }
                        className={cn(
          ...
          <div className="rounded-xl border border-amber-200 bg-amber-50/50 p-4 flex gap-4">
            ...
            <div className="space-y-1">
              <p className="text-sm font-bold uppercase text-amber-800  leading-none">
              ...
              <p className="text-base font-bold text-amber-700 leading-relaxed">
                ...
                <span className="underline">
                  {currentAdviser?.name || "the original adviser"}
                </span>
                .
                {selectedSubstitute && (
                  <>
                    ...
                    will assume official signing authority for all subsequent
                    periods.
                  </>
                )}
              </p>
        ...
        <DialogFooter className="px-6 py-4 bg-muted/20 border-t flex flex-row items-center justify-between gap-4">
          <Button
            ...
            className="font-bold uppercase text-base  h-11 px-8">
            Cancel
          </Button>
          <Button
            onClick={handleHandover}
            disabled={submitting || !substituteTeacherId || !handoverReason}
            className="font-bold uppercase text-base  h-11 px-8 bg-primary shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all">
            {submitting ? (
              <>
              ...
              <>
                <RefreshCcw className="mr-2 h-4 w-4" /> Execute Official
                Handover
              </>
            )}
          </Button>
