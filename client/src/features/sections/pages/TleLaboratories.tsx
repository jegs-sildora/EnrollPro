    ...
    <div
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 space-y-1">
          <p className="font-black uppercase text-sm text-foreground truncate">
            {section.name}
          </p>
          {badgeLabel && (
            <Badge
...
export default function TleLaboratories() {
    ...
    try {
      const [sectionsRes, tleProgramsRes] = await Promise.all([
        api.get(`/sections/${ayId}`, {
          params: { sectionType: "TLE_LABORATORY" },
        }),
      ...
      if (sectionsRes.data.gradeLevels.length > 0) {
        const g9or10 = sectionsRes.data.gradeLevels.find((g: GradeLevelGroup) =>
          TLE_GRADE_DISPLAY_ORDERS.includes(g.displayOrder),
        );
        setActiveGradeId(
          (prev) =>
            prev ||
            String((g9or10 ?? sectionsRes.data.gradeLevels[0]).gradeLevelId),
        );
      }
    } catch {
      sileo.error({
        title: "Unable to load TLE sections",
  ...
  const resolveTleProgram = useCallback(
    (tleProgramId: number | null | undefined): TLEProgram | undefined =>
      tleProgramId != null
        ? tlePrograms.find((p) => p.id === tleProgramId)
        : undefined,
    [tlePrograms],
  );
    ...
    (section: SectionItem, glName: string, glDisplayOrder: number) => {
      ...
      setSectionFormData({
        ...
        adviserId: section.advisingTeacher
          ? String(section.advisingTeacher.id)
          : "none",
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      if (formSheetMode === "create") {
        ...
        sileo.success({
          title: "TLE section created",
          description: `${payload.name} added successfully.`,
        });
      } else {
        await api.put(`/sections/${editingSectionId}`, payload);
        sileo.success({
          title: "TLE section updated",
          description: `Changes to ${payload.name} saved.`,
        });
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      sileo.success({
        ...
        description: `${deleteName} was removed.`,
      });
      setDeleteId(null);
      void fetchData();
    } catch {
    ...
    <div className="p-4 sm:p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-foreground font-bold">
          Manage TLE laboratory sections and advising teachers
        </p>
      </div>
            ...
            {tleGradeGroups.map((g) => (
              <TabsTrigger
                ...
                <span className="relative z-20">
                  {g.gradeLevelName.replace(/grade\s*/i, "Grade ")}
                </span>
              </TabsTrigger>
            ))}
          </TabsList>
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              {tleGradeGroups.map((g) => {
                ...
                const tleSections = g.sections.filter(
                  (s) => s.tleProgramId != null,
                );
      ...
      <SectionFormSheet
        ...
        canSubmit={
          !!sectionFormData.name.trim() && !!sectionFormData.tleProgramId
        }
      ...
      <SectionRosterModal
        ...
        onOpenChange={(open) => {
          if (!open) setRosterSectionId(null);
        }}
      />
    </div>
  );
