...
export function GuardianContact({ applicant }: { applicant: ApplicantDetail }) {
	const c =
		applicant.motherName?.contactNumber ||
		applicant.fatherName?.contactNumber ||
		applicant.guardianInfo?.contactNumber ||
		'N/A';
	const name = applicant.guardianInfo?.firstName
		? `${applicant.guardianInfo.firstName} ${applicant.guardianInfo.lastName}`
		: applicant.motherName?.firstName
			? `${applicant.motherName.firstName} ${applicant.motherName.lastName}`
			: 'N/A';
...
export function Classifications({ applicant }: { applicant: ApplicantDetail }) {
	return (
		<CollapsibleSection title='Classifications'>
			...
			<span>
				{applicant.isIpCommunity ? `YES (${applicant.ipGroupName})` : 'NO'}
			</span>

			<span className='text-muted-foreground'>4Ps:</span>
			<span>
				{applicant.is4PsBeneficiary
					? `YES (${applicant.householdId4Ps})`
					: 'NO'}
			</span>
