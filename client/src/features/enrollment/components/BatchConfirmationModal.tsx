...
export function BatchConfirmationModal({
  ...
  const handleLrnLookup = async (lrn: string) => {
    ...
    if (stagingList.some(s => s.lrn === lrn)) {
      sileo.warning({ title: "Duplicate LRN", description: "This student is already in the staging queue." });
      setLrnInput("");
      return;
    }
  ...
  const updateStagedLearner = (lrn: string, fields: Partial<StagedLearner>) => {
    setStagingList(prev => prev.map(s => s.lrn === lrn ? { ...s, ...fields } : s));
  };
  ...
  const handleCsvUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        ...
        for (const row of rows) {
          ...
          if (lrn && lrn.length === 12) {
             validRows.push({
               ...
               contact: row.CONTACT || "",
               consent: row.CONSENT?.toUpperCase() === "YES" || row.CONSENT === "1"
             });
          } else {
        ...
        sileo.info({ title: "Processing CSV", description: `Fetching metadata for ${validRows.length} learners...` });
        
        for (const v of validRows) {
           await handleLrnLookup(v.lrn);
           updateStagedLearner(v.lrn, { guardianName: v.guardian, contactNumber: v.contact, isEnrolling: v.consent });
        }
        setLoading(false);
      }
    });
  ...
  const handleProcessBatch = async () => {
    ...
    const missingData = stagingList.filter(s => s.isEnrolling && (!s.guardianName || !s.contactNumber));
    ...
    try {
      const payload = {
        batch: stagingList.map(s => ({
          ...
          intakeMethod: activeTab === "scan" ? "BATCH_CONFIRMATION" : "CSV_UPLOAD"
        }))
      };
      ...
      <DialogContent className="max-w-4xl p-0 overflow-hidden border-none shadow-2xl bg-background rounded-2xl">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsList className="bg-muted/50 h-auto p-1.5 gap-1.5 justify-start border border-border/60 relative rounded-xl w-full sm:w-auto">
              ...
              >
                ...
                <span className={cn(
                  ...
                  activeTab === "scan" ? "text-primary-foreground" : "text-muted-foreground hover:text-foreground"
                )}>
              ...
              >
                ...
                <span className={cn(
                  "relative z-20 flex items-center gap-2 transition-colors duration-200", 
                  activeTab === "csv" ? "text-primary-foreground" : "text-muted-foreground hover:text-foreground"
                )}>
          ...
          <div className="p-8 pt-6 space-y-6">
            <TabsContent value="scan" className="m-0 space-y-6">
              <div className="space-y-3">
                ...
                <div className="relative group">
                  ...
                  <Input
                    ref={scanInputRef}
                    value={lrnInput}
                    onChange={(e) => setLrnInput(e.target.value.replace(/\D/g, "").slice(0, 12))}
                    onKeyDown={(e) => e.key === "Enter" && handleLrnLookup(lrnInput)}
            ...
            <TabsContent value="csv" className="m-0">
               <div className="border-2 border-dashed border-border rounded-2xl p-12 flex flex-col items-center justify-center text-center space-y-4 bg-muted/5 hover:bg-muted/10 transition-all cursor-pointer relative group">
                  ...
                  <div className="space-y-1">
                    <p className="font-black uppercase tracking-tight text-xl text-foreground">Drop CSV Enrollment List</p>
                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">
                       Required: LRN, GUARDIAN, CONTACT, CONSENT
                    </p>
                  </div>
                  <Button variant="outline" className="font-black uppercase text-[10px] tracking-widest px-8 shadow-sm">
                    Browse Files
                  </Button>
               </div>
            </TabsContent>
                  ...
                  <div className="max-h-[320px] overflow-y-auto scrollbar-thin">
                    {stagingList.length === 0 ? (
                      <div className="py-24 flex flex-col items-center justify-center text-center space-y-4">
                        ...
                        <div className="space-y-1">
                           <p className="text-sm font-black uppercase tracking-widest text-muted-foreground">Pipeline is Clear</p>
                           <p className="text-xs text-muted-foreground/50 font-bold uppercase tracking-tighter">Ready for multi-scan or file ingestion</p>
                        </div>
                      </div>
                    ) : (
                      <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0 z-10 bg-muted/95 backdrop-blur-md border-b border-border">
                          <tr>
                            <th className="px-5 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-muted-foreground">Learner / Profile</th>
                            <th className="px-5 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-muted-foreground">Guardian Name</th>
                            <th className="px-5 py-4 text-[10px] font-black uppercase tracking-[0.15em] text-muted-foreground">Contact</th>
                            <th className="px-5 py-4 text-center text-[10px] font-black uppercase tracking-[0.15em] text-muted-foreground w-[100px]">Consent</th>
                            <th className="px-5 py-4 w-[60px]"></th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-border/50">
                          {stagingList.map((s) => (
                            <tr key={s.lrn} className={cn(
                              "transition-all duration-200 group",
                              !s.isEnrolling ? "bg-muted/10 grayscale" : "hover:bg-muted/20"
                            )}>
                              <td className="px-5 py-4">
                                <div className="flex flex-col gap-0.5">
                                  <span className="font-black text-sm uppercase text-foreground group-hover:text-primary transition-colors">{s.lastName}, {s.firstName}</span>
                                  <div className="flex items-center gap-2">
                                     <span className="text-[10px] font-black text-muted-foreground tracking-tighter tabular-nums bg-muted/50 px-1.5 py-0.5 rounded border border-border/50">
                                       {s.lrn}
                                     </span>
                                     <Badge variant="outline" className="text-[9px] font-black uppercase h-4 px-1.5 border-primary/20 text-primary">
                                       {s.currentGradeLevel}
                                     </Badge>
                                  </div>
                                </div>
                              ...
                              <td className="px-5 py-4">
                                <Input 
                                  ...
                                  onChange={(e) => updateStagedLearner(s.lrn, { guardianName: e.target.value.toUpperCase() })}
                              ...
                              <td className="px-5 py-4">
                                <Input 
                                  ...
                                  onChange={(e) => updateStagedLearner(s.lrn, { contactNumber: e.target.value.replace(/\D/g, "") })}
                              ...
                              <td className="px-5 py-4">
                                <div className="flex flex-col items-center gap-1.5">
                                  <Switch 
                                    ...
                                    onCheckedChange={(val) => updateStagedLearner(s.lrn, { isEnrolling: val })}
                                    className="data-[state=checked]:bg-emerald-600 scale-90"
                                  />
                                  <span className={cn(
                                    "text-[9px] font-black uppercase tracking-[0.1em]",
                                    s.isEnrolling ? "text-emerald-700" : "text-muted-foreground"
                                  )}>
           ...
           <div className="flex flex-col sm:items-start items-center">
              ...
              <div className="flex items-center gap-3">
                 <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 rounded-lg border border-emerald-100">
                    ...
                    <span className="text-[10px] font-black uppercase text-emerald-800 tracking-tight">Confirmed: {stagingList.filter(s => s.isEnrolling).length}</span>
                 </div>
                 <div className="flex items-center gap-2 px-3 py-1.5 bg-muted/50 rounded-lg border border-border/50">
                    <div className="w-2 h-2 rounded-full bg-muted-foreground" />
                    <span className="text-[10px] font-black uppercase text-muted-foreground tracking-tight">Transfers: {stagingList.filter(s => !s.isEnrolling).length}</span>
                 </div>
              </div>
           </div>
